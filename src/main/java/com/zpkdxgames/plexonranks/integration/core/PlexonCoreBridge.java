...
public final class PlexonCoreBridge implements CoreBridge {
    private static final Set<String> CAPABILITIES = Set.of(
            "rank-engine", "rank-progression", "rank-requirements", "rank-rewards", "rank-api",
            "rankup-events", "sqlite-persistence", "luckperms", "vault", "placeholderapi");
    private static final Set<String> INTEGRATION_CAPABILITIES = Set.of(
            "rank-api", "rankup-event", "rank-change-event", "pre-rankup-event", "rankup-transaction-id", "luckperms-sync");
    ...
    @Override public String registrationState() { return ownsRegistration ? core.modules().find(MODULE_ID).map(d -> d.state().name()).orElse("NOT_REGISTERED") : registrationState; }
    @Override public String detail() { return ownsRegistration ? core.modules().find(MODULE_ID).map(ModuleDescriptor::detail).orElse(detail) : detail; }

    @Override public void registerStarting() {
        ModuleDescriptor descriptor = new ModuleDescriptor(MODULE_ID, "PlexonRanks", plugin.getName(),
                plugin.getPluginMeta().getVersion(), plugin, ModuleVersionRange.parse(SUPPORTED_API_RANGE), CAPABILITIES,
                ModuleState.STARTING, "Initializing PlexonRanks", Instant.now());
        ...
        if (!result.success() && !ownsRegistration) plugin.getLogger().warning("PlexonCore module registration rejected: " + result.message());
        else if (!compatible) plugin.getLogger().warning("PlexonCore API " + version.apiVersion() + " is incompatible with supported range " + SUPPORTED_API_RANGE + "; rank gameplay will continue in standalone compatibility mode.");
    }
    ...
    private void update(ModuleState moduleState, IntegrationState integrationState, String newDetail) {
        ...
        core.integrations().publish("PLEXON_RANKS", plugin.getName(), plugin.getPluginMeta().getVersion(), integrationState, INTEGRATION_CAPABILITIES, newDetail);
    ...
    @Override public void unregister() {
        ...
        if (version.apiMajor() >= 2) core.modules().unregisterOwnedBy(plugin);
        else core.modules().find(MODULE_ID).filter(d -> d.plugin() == plugin).ifPresent(d -> core.modules().unregister(MODULE_ID));
        if (compatible) core.integrations().publish("PLEXON_RANKS", plugin.getName(), plugin.getPluginMeta().getVersion(), IntegrationState.DEGRADED, INTEGRATION_CAPABILITIES, "PlexonRanks is disabled");
        ownsRegistration = false;
